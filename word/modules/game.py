...
from word.database.db import update_stats, get_stats, add_group, get_group, update_longest_word
...
class Game:
    ...
    async def next_turn(self):
        ...
        cache_total_players = {p['id']: p['mention'] for p in self.players}
    ...
    async def handle_timeout(self):
        if not self.players:
            await word.send_message(self.chat_id, "No players remaining. Ending game.")
            del active_games[self.chat_id]
            return
        if self.turn_index >= len(self.players):
            self.turn_index = 0
        ...
        if len(self.players) == 1:
            ...
            if longest and longest != "N/A":
                await update_longest_word(winner['id'], longest)
            del active_games[self.chat_id]
        elif len(self.players) == 0:
            await word.send_message(self.chat_id, "No one left to continue. Game ended.")
            del active_games[self.chat_id]
        else:
...
async def start_classic(client, message: Message):
    ...
    if not await get_group(chat_id):
...
async def extend_game(client, message: Message):
    ...
    game = pending_games[chat_id]
    await message.reply("Game start extended by 30 seconds!")
